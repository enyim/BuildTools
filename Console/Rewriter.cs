...
namespace Enyim.Build
{
	internal class Rewriter
	{
		...
		private ModuleRewriterBase ResolveRewriterInstance(string rewriterPath)
		{
			const string ExpectedTypeName = "ModuleRewriter";

			var context = new AddinLoadContext(rewriterPath, new[]
			{
				"Enyim.Build.Rewriters.Core",
				"Enyim.Build.Rewriters.Extensions",
				"Mono.Cecil",
				"Mono.Cecil.Rocks",
				"Mono.Cecil.Pdb",
			});
			...
			var implementation = context
									.LoadFromAssemblyPath(rewriterPath)
									.ExportedTypes
									.FirstOrDefault(t => t.Name == ExpectedTypeName)
										?? throw new InvalidOperationException($"Cannot load {ExpectedTypeName} from {rewriterPath}");

			var instance = Activator.CreateInstance(implementation) as ModuleRewriterBase
							?? throw new InvalidOperationException($"{implementation.FullName} must inherit from {typeof(ModuleRewriterBase).FullName}");
	...
	//public interface IAddinPathResolver
	//{
	//	string Resolve(string name);
	//}

	//public class DebugAddinPathResolver : IAddinPathResolver
	//{
	...
	//		if (files.Length > 1) throw new FileNotFoundException($"Multiple dlls found for {name}; there should only be one.\n{String.Join("\n", files)}");
